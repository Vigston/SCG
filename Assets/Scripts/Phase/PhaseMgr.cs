...
public class PhaseMgr : MonoBehaviour
{
	...
	// インスタンス
	public static PhaseMgr instance;
	...
	private async UniTask RunTurnCycle(CancellationToken token)
	{
		...
		while (true)
		{
			...
			User playerUser = UserMgr.Instance.GetSetPlayerUser;
			User enemyUser = UserMgr.Instance.GetSetEnemyUser;
			...
			if (!PhotonNetwork.IsMasterClient)
			{
				...
				if (!DebugMgr.Instance.isSingleDebug)
				{
					NetWorkMgr.photonView.RPC(nameof(NetWorkMgr.RPC_PushUser_CM), RpcTarget.AllBuffered, (int)playerUser.GetSetSide, (int)playerUser.GetSetPhaseType, playerUser.GetSetPhaseReadyFlag, playerUser.GetSetGameStartFlag);
				}
			}
			...
			if (PhotonNetwork.IsMasterClient)
			{
				NetWorkMgr.photonView.RPC(nameof(NetWorkMgr.RPC_SyncUser_MC), RpcTarget.OthersBuffered, (int)playerUser.GetSetSide, (int)playerUser.GetSetID, (int)playerUser.GetSetPhaseType, playerUser.GetSetPhaseReadyFlag, playerUser.GetSetGameStartFlag);
				NetWorkMgr.photonView.RPC(nameof(NetWorkMgr.RPC_SyncUser_MC), RpcTarget.OthersBuffered, (int)enemyUser.GetSetSide, (int)enemyUser.GetSetID, (int)enemyUser.GetSetPhaseType, enemyUser.GetSetPhaseReadyFlag, enemyUser.GetSetGameStartFlag);
			}
