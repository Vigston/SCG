...
public class DiceRollPhase : Phase
{
	// 独自のState列挙型
	...
	private async UniTask MainStateAction()
	{
		if (IsFirstState())
		{
			...
			// サイコロを振るアクションを実行
			m_DiceRollAction = ActionMgr.instance.ActivateAction<DiceRollAction>(2);
		}

		// アクションが終わるまで待機
		await ActionMgr.instance.WaitForAction(m_DiceRollAction);
