...
public class MainPhase : Phase
{
	...
	public override async UniTask UpdatePhase(CancellationToken token)
	{
		...
		while ((MainPhaseState)GetSetState != MainPhaseState.TurnEndState)
		{
			...
			// マスタークライアントでアクションを実行
			if (PhotonNetwork.IsMasterClient || DebugMgr.Instance.isSingleDebug)
			{
			...
			{
				CardAbilityMgr abilityManager = CardAbilityMgr.instance;
				// アビリティキューが空になるまで待機
	...
	private async UniTask MainStateAction()
	{
		if (IsFirstState())
		{
			...
			if(PhotonNetwork.IsMasterClient || DebugMgr.Instance.isSingleDebug)
			{
				CardArea targetCardArea = StageMgr.instance.GetCardAreaFromIndex(1);
				m_AssignCardAction = ActionMgr.instance.ActivateAction<AssignCardAction>(targetCardArea.transform);
			}
		}

		await ActionMgr.instance.WaitForAction(m_AssignCardAction);
